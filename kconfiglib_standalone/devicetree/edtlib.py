...
class Binding:
    ...
    def _merge_includes(self, raw: dict, binding_path: Optional[str]) -> dict:
        ...
        if isinstance(include, str):
            ...
            _merge_props(merged, self._load_raw(include), None, binding_path, False)
        elif isinstance(include, list):
            # List of strings and maps. These types may be intermixed.
            for elem in include:
                if isinstance(elem, str):
                    _merge_props(
                        merged, self._load_raw(elem), None, binding_path, False
                    )
                    ...
                    _filter_properties(
                        contents, allowlist, blocklist, child_filter, binding_path
                    )
...
def _slice(node: dtlib_Node, prop_name: str, size: int, size_hint: str) -> List[bytes]:
...
def _map(
    ...
    while raw:
        ...
        if child_spec_entry == masked_child_spec:
            ...
            parent_spec = _pass_thru(prefix, child, parent, child_spec, parent_spec)

            # Found match. Recursively map and return it.
            return _map(
                prefix, parent, map_parent, parent_spec, spec_len_fn, require_controller
            )
...
def _map_interrupt(
    ...
    def own_address_cells(node):
        ...
        if not address_cells:
            _err(
                f"missing #address-cells on {node!r} " "(while handling interrupt-map)"
            )
...
def _map_phandle_array_entry(
    ...
    return _map(
        basename, child, parent, child_spec, spec_len_fn, require_controller=False
    )
...
_DEFAULT_PROP_SPECS: Dict[str, PropertySpec] = {
    name: PropertySpec(name, _DEFAULT_PROP_BINDING) for name in _DEFAULT_PROP_TYPES
}
...
class Node:
    ...
    def children(self) -> Dict[str, "Node"]:
        ...
        return {
            name: self.edt._node2enode[node] for name, node in self._node.nodes.items()
        }
        ...
        for item in _phandle_val_list(prop, specifier_space):
            ...
            res.append(
                ControllerAndData(
                    node=self,
                    controller=controller,
                    data=self._named_cells(controller, mapped_data, specifier_space),
                    name=None,
    ...
    def _init_prop(self, prop_spec: PropertySpec, err_on_deprecated: bool) -> None:
    ...
    def _init_props(
        ...
        if prop2specs:
            ...
            for name in node.props:
                ...
                val = self._prop_val(
                    name, prop_spec.type, False, False, None, None, err_on_deprecated
                )
                self.props[name] = Property(prop_spec, val, self)
        ...
        for prop in pinctrl_props:
            ...
            self.pinctrls.append(
                PinCtrl(
                    node=self,
                    name=None,
                    conf_nodes=[self.edt._node2enode[node] for node in prop.to_nodes()],
                )
            )
