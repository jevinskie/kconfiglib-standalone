...
def referenced_outside_kconfig():
    ...
    for modpath in modules:
        for line in run(
            ("git", "grep", "-h", "-I", "--extended-regexp", regex), cwd=modpath
        ).splitlines():
...
def check_missing_config_prefix():
    ...
    for modpath in modpaths:
        ...
        defines = run(
            ("git", "grep", "--extended-regexp", regex), cwd=modpath, check=False
        )
        # Could pass --only-matching to git grep as well, but it was added
        # pretty recently (2018)
...
C preprocessor magic can trip up this check.""",
    ...
    parser.add_argument(
        ...
        action="append_const",
        dest="checks",
        const=check_pointless_menuconfigs,
        help="""\
...
depends on it""",
    ...
    parser.add_argument(
        ...
        action="append_const",
        dest="checks",
        const=check_defconfig_only_definition,
        help="""\
...
'hex', etc.) removed from Kconfig.defconfig.""",
    ...
    parser.add_argument(
        ...
        action="append_const",
        dest="checks",
        const=check_missing_config_prefix,
        help="""\
...
def modules_file_dir():
    ...
    run(
        (
            os.path.join("scripts", "zephyr_module.py"),
            "--kconfig-out",
            os.path.join(tmpdir, "Kconfig.modules"),
        )
    )
    return tmpdir
